...
// Note: this does not use path compression
// Note: vectors "arr" and "size" should be declared globally

vector<lli>arr(1000007);    // represents the parent node of every node
vector<lli>size(1000007);   // represents the size of the subsets
...
bool find(lli a, lli b){
    ...
    if(root(a)==root(b)){   //if A and B have same root, that means that they are connected
        return true;
